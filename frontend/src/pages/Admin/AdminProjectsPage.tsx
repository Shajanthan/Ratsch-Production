...
function formatMainCategoryLabel(key?: string): string {
  const k = (key ?? "").trim();
  if (!k) return "—";
      ...
      <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-lg p-6 md:p-8">
        ...
        <div className="flex items-center justify-between gap-2 mb-6 border-b border-white/10 pb-4">
          <div className="flex flex-wrap gap-2">
            <span className="text-sm uppercase text-white/70">Projects</span>
          </div>
          <button
                    ...
                    <tbody>
                      {paginatedProjects.map((p) => (
                        ...
                        >
                          ...
                          <td className="py-3 px-2 text-white/80 hidden md:table-cell">
                            {formatMainCategoryLabel(p.navbarCategoryKey)}
                          </td>
                          <td className="py-3 px-2 text-white/80 hidden lg:table-cell">
                            {p.navbarSubItem?.trim() ||
